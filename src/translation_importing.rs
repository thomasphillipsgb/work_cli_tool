...
pub fn import(csv_path: &str, resx_path: &str) -> Result<(), String> {
    // Remove surrounding quotes if present
    let csv_path = csv_path.trim_matches('"');
    let csv_path = csv_path.trim_matches('"');
    // Read CSV file
    let csv_content =
        std::fs::read_to_string(csv_path).map_err(|e| format!("Failed to read CSV file: {}", e))?;
    ...
    writeln!(resx_file, r#"<root>"#).map_err(|e| format!("Failed to write to RESX file: {}", e))?;
    // Process CSV lines
